...
Input Format
	...
	The OCCUPATIONS table is described as follows: Occupation will only contain one 
	of the following values: Doctor, Professor, Singer or Actor.
...
Explanation
	...
	corresponding to each profession (2 <= 2 <= 3 <= 3), and then alphabetically by 
...
SELECT CONCAT('There are total ',COUNT(NAME),
              ' ',LOWER(OCCUPATION),'s.')
FROM OCCUPATIONS
GROUP BY OCCUPATION
